jQuery(function($){
  ...
  $('.show-block .nav a[href^=#]').live( 'click', function(e){
    var state = {},
      
      // Get the id of this .bbq widget.
      id = $(this).closest( '.show-block' ).attr( 'id' ),
  ...
  $(window).bind( 'hashchange', function(e) {
    ...
    $('.show-block').each(function(){
      ...
      that.find( 'a.show-block-current' ).removeClass( 'show-block-current' );
      
      // Hide any visible ajax content.
      that.find( '.show-block-current' ).children( ':visible' ).hide();
      
      // Add .bbq-current class to "current" nav link(s), only if url isn't empty.
      url && that.find( 'a[href="#' + url + '"]' ).addClass( 'show-block-current' );
